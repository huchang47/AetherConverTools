...
# 载入样板图像
im = Image.open(image_template)
encoded_image_template = img_str(im)
...
for frame, txt in zip(frame_files, txt_files):
    ...
    # 定义一个ContrlNet参数表
    control_nets = [
        ("lineart_realistic", 0.6,encoded_image), # 默认为不调用任何CN，避免没有模型报错。有能力的自己改：CN名称和权重，多个CN就同样加一行。
        ("tile_colorfix", 0.6,encoded_image),
        ("reference_only",1.0,encoded_image_template),
    ]
    # 定义ADetailer的参数
    Ade_args = [
        {
    ...
    encoded_image_template = encoded_image  #当前生成图片存成模版
    print(frame+"生成完毕！")
